...
4. If exit code is 0 and `projDirClaude` exists: **the exit-code gate is correct HERE and must stay.** 14.4 forbids gating only NEW-SESSION REGISTRATION (11.6.2), which it names explicitly. Fork detection is different: a resume that FAILED must not swap the registered GUID onto whatever transcript happens to be newest, which is the same adoption hazard that bit the bash launch path. An earlier edit on 2026-08-27 wrongly removed this gate from the spec; it was restored the same day.
...
4. If `projDirClaude` exists, **regardless of exit code**: this step used to read "if exit code is 0 and", which contradicted 14.4 and both shipped implementations. Gating detection on the exit code is precisely what made new sessions vanish whenever the user exited any way other than `/exit`. Corrected 2026-08-27.
