...
$realistic = @'
{
  ...
  "hooks": {
    ...
    "PreToolUse": [
      ...
      { "hooks": [ { "type": "command", "command": "python \"~/.claude/hooks/guard-worklog.py\"", "timeout": 10 } ] }
    ],
    "UserPromptSubmit": [
      { "hooks": [ { "type": "command", "command": "python \"~/.claude/hooks/record-prompt.py\"", "timeout": 10 } ] }
    ],
    "Stop": [
...
Check 'strip: guard-worklog is gone' ($after -notmatch 'guard-worklog')
Check 'strip: record-prompt is gone (a UserPromptSubmit hook, not PreToolUse)' ($after -notmatch 'record-prompt')
